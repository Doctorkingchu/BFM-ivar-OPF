...
# Continuous fallback for relaxed subproblem
USE_IPOPT_RELAXED_FALLBACK = True
IPOPT_MAX_ITER = 4000
IPOPT_TOL = 1e-6
    ...
    try:
        for i in data["buses"]:
            if m.v[i].value is None:
                return False
        for g in m.G:
            if m.Pg[g].value is None or m.Qg[g].value is None:
                return False
        for (i, j) in data["E"]:
            ...
            if m.s_vdrop[i, j].value is None:
                return False
        for i in data["buses"]:
            if m.sP_pos[i].value is None or m.sP_neg[i].value is None:
                return False
            if m.sQ_pos[i].value is None or m.sQ_neg[i].value is None:
                return False
        return True
    except Exception:
        return False
...
def _recommended_tap_choice(data: Dict[str, Any], i: int, j: int) -> int:
    rec = data.get("recommended_taps", {})
    if (int(i), int(j)) in rec:
        return int(rec[(int(i), int(j))])
    return _default_tap_choice([int(t) for t in data["K"][(int(i), int(j))]])


def _recommended_shunt_choice(data: Dict[str, Any], i: int) -> int:
    rec = data.get("recommended_shunts", {})
    if int(i) in rec:
        return int(rec[int(i)])
    return 0


def make_initial_warm(data: Dict[str, Any]) -> Dict[str, Any]:
    warm = {
        "v": {},
        "Pij": {},
        "Qij": {},
    ...
    slack_v_sq = float(data["slack_vm_pu"]) ** 2
    ...
    for i in data["buses"]:
        if int(i) == slack:
            ...
            v0 = 1.0
        warm["v"][int(i)] = _clip(v0, float(data["Vmin_pu"][int(i)]) ** 2, float(data["Vmax_pu"][int(i)]) ** 2)

    for (i, j) in data["E"]:
        warm["Pij"][(int(i), int(j))] = 0.0
        warm["Qij"][(int(i), int(j))] = 0.0

    for g, rec in enumerate(data["gen_records"]):
        pmin = float(rec["pmin_pu"])
        pmax = float(rec["pmax_pu"])
        qmin = float(rec["qmin_pu"])
        qmax = float(rec["qmax_pu"])
        warm["Pg"][int(g)] = _clip(0.0, pmin, pmax)
        warm["Qg"][int(g)] = _clip(0.0, qmin, qmax)

    for i in data.get("C", []):
        warm["a_sh"][int(i)] = float(_recommended_shunt_choice(data, int(i)))

    for (i, j) in data.get("T", []):
        t0 = _recommended_tap_choice(data, int(i), int(j))
        taps = [int(t) for t in data["K"][(int(i), int(j))]]
        if t0 not in taps:
            t0 = _default_tap_choice(taps)
        for tap in taps:
            warm["beta"][(int(i), int(j), int(tap))] = 1.0 if int(tap) == int(t0) else 0.0

    return warm


def _merge_warm_with_defaults(data: Dict[str, Any], warm: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    base = make_initial_warm(data)
    if warm is None:
        return base

    for key in ["v", "Pij", "Qij", "Pg", "Qg", "beta", "a_sh"]:
        if key in warm and isinstance(warm[key], dict):
            base[key].update(warm[key])
    return base
...
def solve_with_ipopt(model: pyo.ConcreteModel, tee: bool) -> bool:
    if not USE_IPOPT_RELAXED_FALLBACK:
        return False
    ...
    try:
        ...
        opt.options["acceptable_tol"] = max(1e-5, float(IPOPT_TOL) * 10.0)
    ...
    try:
        opt.solve(model, tee=tee, load_solutions=True)
    except Exception:
        return False
...
def solve_subproblem_robust(
    ...
    seeded_warm = _merge_warm_with_defaults(data, warm)

    # 1) binary attempts
    attempts_bin = [
        ("binary", warm),
        ("binary_seeded", seeded_warm),
        ("binary_cold", None),
    ]
    for tag, w in attempts_bin:
        m_bin = build_subproblem(data, ell_fix, relax_binaries=False, warm=w)
        if solve_with_scip(m_bin, SCIP_TIME_LIMIT, SCIP_GAP_LIMIT, tee if tag == "binary" else False):
            return m_bin, tag

    if tee:
        print("[WARN] binary infeasible/no incumbent -> RELAXED ...")

    # 2) relaxed continuous fallback
    attempts_relax = [
        ("relaxed_ipopt", seeded_warm, "ipopt"),
        ("relaxed_scip", seeded_warm, "scip"),
        ("relaxed_cold_ipopt", None, "ipopt"),
        ("relaxed_cold_scip", None, "scip"),
    ]
    for tag, w, solver_name in attempts_relax:
        m_relax = build_subproblem(data, ell_fix, relax_binaries=True, warm=w)
        ok = solve_with_ipopt(m_relax, False) if solver_name == "ipopt" else solve_with_scip(m_relax, SCIP_TIME_LIMIT, SCIP_GAP_LIMIT, False)
        if ok:
            return m_relax, tag

    raise RuntimeError("No feasible/complete incumbent found in binary or relaxed subproblem.")
...
def run_bfm_ag(data: Dict[str, Any], max_iters: int, eps: float, tee: bool) -> Dict[str, Any]:
    ...
    for t in range(1, max_iters + 1):
        ...
        try:
            ...
            status = "subproblem_failed"
            message = str(e)
            print(f"[EARLY-STOP] t={t}: {message}")
            break
        ...
        warm = {
            ...
            "Pij": dict(P_sol),
            "Qij": dict(Q_sol),
            "Pg": {int(g): _val(model.Pg[g], 0.0) for g in model.G},
            "Qg": {int(g): _val(model.Qg[g], 0.0) for g in model.G},
            "a_sh": {int(i): _val(model.a_sh[i], 0.0) for i in data["C"]} if len(data["C"]) > 0 else {},
            "beta": {(i, j, tap): _val(model.beta[i, j, tap], 0.0) for (i, j, tap) in model.BETA_INDEX} if len(data["T"]) > 0 else {},
        }
    ...
    return {"best": best, "last": last, "status": status, "message": message}
...
def main():
    ...
    print("[INFO] Data summary (IEEE 300-bus, BFMag/BFM-ag)")
    ...
    if best["model"] is not None:
        ...
        print("[WARN] No solved subproblem was obtained, so detailed report is skipped.")
