# DCOPF.py
# ------------------------------------------------------------
# Standalone Pyomo MIQP DC-OPF on the IEEE 3012-bus system
# with OLTC tap selection (one-hot) via Big-M.
#
# This file intentionally depends only on:
#   - ieee3012bus.py   (network builder / metadata)
#   - Pyomo + SCIP
#
...
RESULT_LOG_PATH = Path(__file__).resolve().with_name("DCOPF_results.txt")
...
def _row_smax_mva(row: dict) -> float:
    if "synthetic_smax_mva" in row:
        return float(row["synthetic_smax_mva"])
    if "smax_mva_used" in row:
        return float(row["smax_mva_used"])
    raise KeyError("branch_params_pu_table is missing both 'synthetic_smax_mva' and 'smax_mva_used'.")
...
def _merge_parallel_series_equivalent(rows: List[dict]) -> Tuple[float, float, float]:
    ...
    for row in rows:
        ...
        if abs(z) <= 1e-12:
            ...
            y = 1.0 / z
        ysum += y
        ssum += max(0.0, _row_smax_mva(row))
...
def extract_dc_miqp_data(
    ...
    """
    Extract MIQP DCOPF data directly from ieee3012bus metadata.
...
def build_miqp_dcopf_model(data: Dict[str, Any]) -> pyo.ConcreteModel:
    ...
    model = pyo.ConcreteModel(name="DC_MIQP_3012bus_standalone")
